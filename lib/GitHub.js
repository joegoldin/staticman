...
class GitHub extends GitService {
  constructor(options = {}) {
    ...
    return (async () => {
      const isAppAuth =
        config.get("githubAppID") && config.get("githubPrivateKey");
      const isLegacyAuth =
        config.get("githubToken") && ["1", "2"].includes(options.version);
      ...
      if (options.oauthToken) {
        ...
        authToken = await this._authenticate(
          options.username,
          options.repository
        );
      } else {
  ...
  writeFileAndSendReview(filePath, data, branch, commitTitle, reviewBody) {
    return super
      .writeFileAndSendReview(filePath, data, branch, commitTitle, reviewBody)
      .catch((err) =>
        Promise.reject(errorHandler("GITHUB_CREATING_PR", { err }))
      );
  }
  ...
  getCurrentUser() {
    return this.api.users
      ...
      .then(normalizeResponse)
      .then(
        ({ login, email, avatar_url, name, bio, company, blog }) =>
